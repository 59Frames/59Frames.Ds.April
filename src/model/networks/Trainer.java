...
public class Trainer {
    ...
    private static double decayRate = 0.999;
    private static double smoothEpsilon = 1e-8;
    private static double gradientClipValue = 5;
    private static double regularization = 0.000001; // L2 regularization strength

    public static double train(int trainingEpochs, double learningRate, NetworkModel model, DataSet data, int reportEveryNthEpoch, Random rng) throws Exception {
        return train(trainingEpochs, learningRate, model, data, reportEveryNthEpoch, false, false, null, rng);
    }

    public static double train(int trainingEpochs, double learningRate, NetworkModel model, DataSet data, int reportEveryNthEpoch, boolean initFromSaved, boolean overwriteSaved, String savePath, Random rng) throws Exception {
        System.out.println("--------------------------------------------------------------");
        if (initFromSaved) {
    ...
    public static Promise<Double> trainAsync(int trainingEpochs, double learningRate, NetworkModel model, DataSet data, int reportEveryNthEpoch, boolean initFromSaved, boolean overwriteSaved, String savePath, Random rng) {
        return new Promise<>(() -> {
            try {
                return train(trainingEpochs, learningRate, model, data, reportEveryNthEpoch, initFromSaved, overwriteSaved, savePath, rng);
            } catch (Exception e) {
                return 1.0;
            }
        });
    ...
    public static double pass(double learningRate, NetworkModel model, List<DataSequence> sequences, boolean applyTraining, Loss lossTraining, Loss lossReporting) throws Exception {
        ...
        for (DataSequence seq : sequences) {
            ...
            for (DataStep step : seq.getSteps()) {
    ...
    public static void updateModelParams(NetworkModel model, double stepSize) throws Exception {
