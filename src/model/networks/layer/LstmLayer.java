...
public class LstmLayer implements NetworkModel {
    ...
    public Matrix forward(@NotNull Matrix input, @NotNull Graph g) throws Exception {
        ...
        Matrix inputGate = g.applyNonlinearity(fInputGate, g.add(g.add(sum0, sum1), bi));

        //forget gate
        Matrix sum2 = g.mul(Wfx, input);
        Matrix sum3 = g.mul(Wfh, hiddenContext);
        Matrix forgetGate = g.applyNonlinearity(fForgetGate, g.add(g.add(sum2, sum3), bf));

        //output gate
        Matrix sum4 = g.mul(Wox, input);
        Matrix sum5 = g.mul(Woh, hiddenContext);
        Matrix outputGate = g.applyNonlinearity(fOutputGate, g.add(g.add(sum4, sum5), bo));

        //write operation on cells
        Matrix sum6 = g.mul(Wcx, input);
        Matrix sum7 = g.mul(Wch, hiddenContext);
        Matrix cellInput = g.applyNonlinearity(fCellInput, g.add(g.add(sum6, sum7), bc));
        ...
        Matrix output = g.elmul(outputGate, g.applyNonlinearity(fCellOutput, cellAct));
